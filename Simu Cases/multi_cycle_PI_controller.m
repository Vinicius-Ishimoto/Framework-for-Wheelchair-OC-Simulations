function output = multi_cycle_PI_controller(PAR,ncycle,speed,dist,varargin)
...
ti = -120*pi/180;
medtime = dist/(speed*(ncycle*2+1));
mintime = medtime*0.5;

Tol = 1e-0;
addParameter(p,'Guess',-1);
addParameter(p,'type','fixed');
addParameter(p,'Max_control',inf);
...
for q = 1:ncycle
	if cont==1
		...
		setup.bound.lower.phase(cont).initial.state = [0,0];
		setup.bound.upper.phase(cont).initial.state = [0,0];
		setup.bound.lower.phase(cont).initial.time = 0;
		setup.bound.upper.phase(cont).initial.time = 0;
        if not(isFixed)
            setup.bound.lower.phase(cont).initial.position = ti;
		...
		setup.bound.lower.phase(cont).initial.state = [0,-inf];
		setup.bound.upper.phase(cont).initial.state = [inf,inf];
		setup.bound.lower.phase(cont).initial.time = 0;
	...
	setup.bound.lower.phase(cont).final.state = [0,-inf];
	...
	setup.bound.lower.phase(cont).state = [0,-inf];
	...
	setup.bound.upper.phase(cont).final.state = [inf,inf];
	...
	setup.bound.upper.phase(cont).state = [inf,inf];
	...
	if hasFixedStart
		ub = [ub,zeros(1,13),  inf,  inf, 0, zeros(1,4),    inf];
		lb = [lb,zeros(1,13),mintime,mintime, 0, zeros(1,4),Contact];
	else
		ub = [ub,zeros(1,13),  inf,  inf, inf, zeros(1,4),    inf];
		lb = [lb,zeros(1,13),mintime,mintime,-inf, zeros(1,4),Contact];
	end
	cont = cont + 1;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
setup.bound.lower.phase(cont).position = PAR.thetac1;
setup.bound.lower.phase(cont).velocity = 0;
setup.bound.lower.phase(cont).state = [-inf,-inf];
setup.bound.lower.phase(cont).control = [0,0,0,-pi/2,0,0,-inf,-inf,0,0];
setup.bound.lower.phase(cont).path = [0,0,0,0,0,0,-Tol];
...
setup.bound.upper.phase(cont).path = [pi,0,0,0,0,0,Tol];
if hasFixedStart
	setup.bound.lower.pconstraints = [lb,0.1,speed,  1,Contact,0];
	setup.bound.upper.pconstraints = [ub,  inf,speed,inf,inf,0];
else
	setup.bound.lower.pconstraints = [lb,0.1,speed,  1,Contact,-inf];
	setup.bound.upper.pconstraints = [ub,  inf,speed,inf,inf,inf];
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	for q = 1:ncycle
        ...
        if not(isfield(opt.solution.phase(cont),'state'))
            ...
            setup.initial_guess.phase(cont).state = opt.solution.phase(cont).state(:,1:2);
        end
        if size(opt.solution.phase(cont).control, 2) == 10
		...
		setup.initial_guess.phase(cont).control = [opt.solution.phase(cont).control(:,1:2),opt.solution.phase(cont).control(:,3:4), ...
                                            ...
                                            PAR.Kt*opt.solution.phase(cont).state(:,3),opt.solution.phase(cont).state(:,3),opt.solution.phase(cont).state(:,3)];
        end
        cont = cont + 1;
		...
		setup.initial_guess.phase(cont).position = opt.solution.phase(cont).position;
		setup.initial_guess.phase(cont).velocity = opt.solution.phase(cont).velocity;
		if size(opt.solution.phase(cont).control, 2) == 7
            setup.initial_guess.phase(cont).control = opt.solution.phase(cont).control;
        else
            setup.initial_guess.phase(cont).control = [opt.solution.phase(cont).control(:,1:2),opt.solution.phase(cont).control(:,3:4), ...
                PAR.Kt*opt.solution.phase(cont).state(:,3),opt.solution.phase(cont).state(:,3),opt.solution.phase(cont).state(:,3)];
        ...
        if isfield(opt.solution.phase(cont),'state')
            setup.initial_guess.phase(cont).state = opt.solution.phase(cont).state(:,1:2);
        else
            setup.initial_guess.phase(cont).state = [opt.solution.phase(cont).velocity*PAR.Rr,zeros(numel(opt.solution.phase(cont).velocity),1)];
        end
		cont = cont + 1;
	end
	setup.initial_guess.phase(cont).time = opt.solution.phase(cont).time;
...
tp1 = input.phase(cont).control(:,7);
u1 = input.phase(cont).control(:,8);
uv1p = input.phase(cont).control(:,9);
uv1n = input.phase(cont).control(:,10);
...
l_c1 = Kte*(Jtotal*err1+Btotal*i_err1);
K = PHI'*M1*dPHI*dtheta1;
% tau_p1 = 100*exp(-((alpha1-beta1)/0.2).^2);
output.phase(cont).RightHandSide = PHI'*(H1*[tau_s1;tau_e1]+ke1-k1)-K+u1;
...
output.phase(cont).derivatives = [(1/Mi)*(tp1/input.auxdata.PAR.R-Ci*v_imp1),err1];
output.phase(cont).path = [q1(:,1)-q1(:,2), ...
    ...
    Tp1-tp1, du1, l_c1 - uv1, uv1p*uv1n ];
...
obj = obj + fator*integrate(input.phase(cont).integrand, u1.^2);
cont = cont + 1;
...
u2 = input.phase(cont).control(:,5);
uv2p = input.phase(cont).control(:,6);
uv2n = input.phase(cont).control(:,7);
uv2 = uv2p - uv2n;
...
l_c2 = Kte*(Jtotal*err2+Btotal*i_err2);

output.phase(cont).RightHandSide = H2*[tau_s2;tau_e2] + ke2 -k2 + [0;0;u2];
...
output.phase(cont).path = [q2(:,1)-q2(:,2), du2, l_c2 - uv2, uv2p*uv2n];
...
p_constraints = [p_constraints, input.phase(cont).initial.time-input.phase(cont-1).final.time]; % n = 1
p_constraints = [p_constraints,input.phase(cont).initial.position(3)-input.phase(cont-1).final.position, ...
	...
	theta_initial - input.phase(cont-1).initial.position]; % n = 16
...
p_constraints = [p_constraints,input.phase(cont-1).final.state-input.phase(cont).initial.state, ...
	...
	input.phase(cont-1).final.position-input.phase(cont-1).initial.position]; % dn = 5
...
obj = obj + fator*integrate(input.phase(cont).integrand, u2.^2);
...
l_c3 = Kte*(Jtotal*err3+Btotal*i_err3);
...
dv_imp3 = 1/Mi*(tp3/input.auxdata.PAR.R-Ci*v_imp3);
...
% obj = obj + integrate(input.phase(cont).integrand,input.phase(cont).control(:,1).^indice+input.phase(cont).control(:,2).^indice)./(input.phase(cont).final.position-input.phase(cont).initial.position);
% obj = obj + integrate(input.phase(cont).integrand,input.phase(cont).control(:,5).^indice+input.phase(cont).control(:,6).^indice)./(input.phase(cont).final.position-input.phase(cont).initial.position);
obj = obj + fator*integrate(input.phase(cont).integrand, u3.^2);
...
output.phase(cont).path = [q3(:,1)-q3(:,2), ...
    ...
    Tp3 - tp3, du3, l_c3 - uv3, uv3p*uv3n ];

theta_total = (theta_total+input.phase(cont).final.position-input.phase(cont).initial.position);
mean_speed = theta_total*input.auxdata.PAR.Rr./input.phase(cont).final.time;
...
output.objective = obj./(input.phase(cont).final.position-input.phase(cont).initial.position);
