function output = multi_cycle_PID_controller_II(PAR,ncycle,speed,dist,varargin)
...
for q = 1:ncycle
	if cont==1
		...
		setup.bound.lower.phase(cont).initial.state = [0,0,0,0,-inf];
		setup.bound.upper.phase(cont).initial.state = [0,0,0,0,inf];
		...
		setup.bound.lower.phase(cont).initial.state = [0,-inf,-MCurr,-inf,-inf];
		setup.bound.upper.phase(cont).initial.state = [inf,inf,MCurr,inf,inf];
		setup.bound.lower.phase(cont).initial.time = 0;
	...
	setup.bound.lower.phase(cont).final.state = [0,-inf,-MCurr,-inf,-inf];
	...
	setup.bound.lower.phase(cont).state = [0,-inf,-MCurr,-inf,-inf];
	...
	setup.bound.upper.phase(cont).final.state = [inf,inf,MCurr,inf,inf];
	...
	setup.bound.upper.phase(cont).state = [inf,inf,MCurr,inf,inf];
	...
	if hasFixedStart
		ub = [ub,zeros(1,13),  inf,  inf,zeros(1,10),    inf,0];
		lb = [lb,zeros(1,13),0.001,0.001,zeros(1,10),Contact,0];
	else
		ub = [ub,zeros(1,13),  inf,  inf,zeros(1,10),    inf,inf];
		lb = [lb,zeros(1,13),0.001,0.001,zeros(1,10),Contact,-inf];
	end
	cont = cont + 1;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
setup.bound.lower.phase(cont).position = PAR.thetac1;
setup.bound.lower.phase(cont).velocity = 0;
setup.bound.lower.phase(cont).state = [-inf,-inf,-MCurr,-inf,-inf];
setup.bound.lower.phase(cont).control = [0,0,0,-pi/2,0,0,-inf];
setup.bound.lower.phase(cont).path = [0,0,0,0];
...
if not(isfield(opt.solution.phase(1),'state'))
	...
	for q = 1:ncycle
		...
		setup.initial_guess.phase(cont).state = [opt.solution.phase(cont).velocity*PAR.Rr,zeros(numel(opt.solution.phase(cont).velocity),4)];
	...
	setup.initial_guess.phase(cont).position = opt.solution.phase(cont).position;
	setup.initial_guess.phase(cont).velocity = opt.solution.phase(cont).velocity;
	setup.initial_guess.phase(cont).state = [opt.solution.phase(1).velocity*PAR.Rr,zeros(numel(opt.solution.phase(1).velocity),4)];
	setup.initial_guess.phase(cont).control = [opt.solution.phase(cont).control(:,1:2),opt.solution.phase(cont).control(:,3:4), ...
												opt.solution.phase(cont).control(:,5:6),opt.solution.phase(cont).control(:,7)];
    ...
    lobo = 1;
    hibo = 0;
    for w = 1:numel(opt.solution.phase)
        hibo = hibo+numel(opt.solution.phase(w).time)-1;
        ctrlaw{w} = rt.ctrlaw(lobo:hibo+1);
        lobo = hibo + 1;
    end
	...
	for q = 1:ncycle
		...
		setup.initial_guess.phase(cont).position = opt.solution.phase(cont).position;
		setup.initial_guess.phase(cont).velocity = opt.solution.phase(cont).velocity;
		setup.initial_guess.phase(cont).state = [opt.solution.phase(cont).state,ctrlaw{cont}];
...
Pr = input.auxdata.PAR.R;
Rr = input.auxdata.PAR.Rr;
fator = 1;
...
dl_c1 = input.phase(cont).state(:,5);
...
rhs = (M1*qdd1-(H1*[tau_s1;tau_e1]+ke1-k1+[0;0;Kmotor*i_motor1]));
% Fcont = G1\rhs;
tp1 = rhs(3);
dctrlaw1 = (1/Td)*(d_err1-dl_c1);
...
output.phase(cont).derivatives = [(1/Mi)*(u1/input.auxdata.PAR.R-Ci*v_imp1),err1, ...
    (1/La)*(l_c1-Ke*dtheta1-Ra*i_motor1), (1/input.auxdata.Td)*(err1-d_err1), dctrlaw1];
...
% obj = obj + fator*integrate(input.phase(cont).integrand,dctrlaw1.^2);
cont = cont + 1;
...
dl_c2 = input.phase(cont).state(:,5);
...
output.phase(cont).derivatives = [ddx2,err2, ...
    (1/La)*(l_c2-Ke*qd2(3)-Ra*i_motor2), (1/input.auxdata.Td)*(err2-d_err2), dctrlaw2];
...
% obj = obj + fator*integrate(input.phase(cont).integrand,dctrlaw2.^2);
...
dl_c3 = input.phase(cont).state(:,5);
...
% derr1 = ddx1/input.auxdata.PAR.Rr-ddtheta1;
% dFc1 = Kte*(M1(end)*derr1+input.auxdata.PAR.dFric*input.auxdata.PAR.Rr^2*err1)/input.auxdata.PAR.Kt;
rhs = (M3*qdd3-(H3*[tau_s3;tau_e3]+ke3-k3+[0;0;Kmotor*i_motor3]));
% Fcont = G1\rhs;
tp3 = rhs(3);
dctrlaw3 = (1/Td)*(d_err3-dl_c3);
% tp3 = M3(3,:)*qdd3-ke3(3)+k3(3)-Kmotor*i_motor3;

obj = obj + integrate(input.phase(cont).integrand,input.phase(cont).control(:,1).^indice+input.phase(cont).control(:,2).^indice);
obj = obj + integrate(input.phase(cont).integrand,input.phase(cont).control(:,5).^indice+input.phase(cont).control(:,6).^indice);
% obj = obj + fator*integrate(input.phase(cont).integrand,dctrlaw3.^2);

output.phase(cont).derivatives = [dv_imp3,err3, ...
    (1/La)*(l_c3-Ke*dtheta3-Ra*i_motor3), (1/input.auxdata.Td)*(err3-d_err3), dctrlaw3];
