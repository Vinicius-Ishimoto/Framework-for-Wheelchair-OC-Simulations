function output = SS_muscledyn_wheelchair(PAR,speed,varargin)
...
setup.bound.upper.phase(cont).initial.state = [1,1,1,1];
...
setup.bound.lower.phase(cont).final.state = [0,0,0,0];
setup.bound.upper.phase(cont).final.state = [1,1,1,1];
...
setup.bound.lower.phase(cont).state = [0,0,0,0];
setup.bound.upper.phase(cont).state = [1,1,1,1];
...
pcu = [zeros(1,13+8),inf,    inf,+hFT];
pcl = [zeros(1,13+8),0.1,Contact,-hFT];
...
if size(optr.solution.phase(1).control,2) == 11
	setup.initial_guess.phase(1).control = [optr.solution.phase(1).control];
else
	setup.initial_guess.phase(1).control(:,1) = optr.solution.phase(1).control(:,1)/max(optr.solution.phase(1).control(:,1)+0.01);
	setup.initial_guess.phase(1).control(:,2) = optr.solution.phase(1).control(:,2)/max(optr.solution.phase(1).control(:,2)+0.01);
	setup.initial_guess.phase(1).control(:,3:4) = optr.solution.phase(1).control(:,3:4);
	setup.initial_guess.phase(1).control(:,5) = optr.solution.phase(1).control(:,5)/max(optr.solution.phase(1).control(:,5)+0.01);
	setup.initial_guess.phase(1).control(:,6) = optr.solution.phase(1).control(:,6)/max(optr.solution.phase(1).control(:,6)+0.01);
	setup.initial_guess.phase(1).control(:,7) = optr.solution.phase(1).taup;
	setup.initial_guess.phase(1).control(:,8:9) = optr.solution.phase(1).control(:,1:2);
	setup.initial_guess.phase(1).control(:,10:11) = optr.solution.phase(1).control(:,5:6);   
end
switch optr.Options.Type
    case 'MSC-PI_Controller'
        setup.initial_guess.phase(1).state(:,1) = optr.solution.phase(1).control(:,1);
        setup.initial_guess.phase(1).state(:,2) = optr.solution.phase(1).control(:,2);
        setup.initial_guess.phase(1).state(:,3) = optr.solution.phase(1).control(:,5);
        setup.initial_guess.phase(1).state(:,4) = optr.solution.phase(1).control(:,6);
    case 'MSC-Reference'
        setup.initial_guess.phase(1).state(:,1) = optr.solution.phase(1).control(:,1);
        setup.initial_guess.phase(1).state(:,2) = optr.solution.phase(1).control(:,2);
        setup.initial_guess.phase(1).state(:,3) = optr.solution.phase(1).control(:,5);
    ...
    case 'MSD-PI_Controller'
        setup.initial_guess.phase(1).state(:,1:4) = optr.solution.phase(1).state(:,3:6);
    otherwise
...
if size(optr.solution.phase(2).control,2) == 8
	setup.initial_guess.phase(2).control = [optr.solution.phase(2).control];
else
	setup.initial_guess.phase(2).control(:,1) = optr.solution.phase(2).control(:,1)/max(optr.solution.phase(2).control(:,1)+0.01);
	setup.initial_guess.phase(2).control(:,2) = optr.solution.phase(2).control(:,2)/max(optr.solution.phase(2).control(:,2)+0.01);
	setup.initial_guess.phase(2).control(:,3) = optr.solution.phase(2).control(:,3)/max(optr.solution.phase(2).control(:,3)+0.01);
	setup.initial_guess.phase(2).control(:,4) = optr.solution.phase(2).control(:,4)/max(optr.solution.phase(2).control(:,4)+0.01);
	setup.initial_guess.phase(2).control(:,5:8) = optr.solution.phase(2).control(:,1:4);   
end
switch optr.Options.Type
    case 'MSC-PI_Controller'
        setup.initial_guess.phase(2).state(:,1) = optr.solution.phase(2).control(:,1);
        setup.initial_guess.phase(2).state(:,2) = optr.solution.phase(2).control(:,2);
        setup.initial_guess.phase(2).state(:,3) = optr.solution.phase(2).control(:,3);
        setup.initial_guess.phase(2).state(:,4) = optr.solution.phase(2).control(:,4);
    case 'MSC-Reference'
        setup.initial_guess.phase(2).state(:,1) = optr.solution.phase(2).control(:,1);
        setup.initial_guess.phase(2).state(:,2) = optr.solution.phase(2).control(:,2);
        setup.initial_guess.phase(2).state(:,3) = optr.solution.phase(2).control(:,3);
        setup.initial_guess.phase(2).state(:,4) = optr.solution.phase(2).control(:,4);
    case 'MSD-PI_Controller'
        setup.initial_guess.phase(2).state(:,1:4) = optr.solution.phase(2).state(:,3:6);
    otherwise
...
[M1,ke1,~,H1,k1] = four_bar_system(q1, qd1,input.auxdata.PAR,'Type','Reference');
