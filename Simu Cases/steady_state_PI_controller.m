function output = steady_state_PI_controller(PAR,speed,varargin)
...
addParameter(p,'OnlyPos',false,@islogical);
parse(p,varargin{:});
...
setup.auxdata.OnlyPos = p.Results.OnlyPos;
...
setup.bound.lower.phase(cont).initial.state = [0,-inf,-inf];
...
setup.bound.lower.phase(cont).final.state = [0,-inf,-inf];
...
setup.bound.lower.phase(cont).state = [0,-inf,-inf];
setup.bound.lower.phase(cont).control = [0,0,0,-pi/2,0,0,-inf,-inf,0,0];
setup.bound.lower.phase(cont).path = [0,0,0,0,0,0,-Tol];
setup.phase(cont).mesh_points = 8;
setup.phase(cont).mesh_number = 8;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% setup.bound.upper.phase(cont).initial.position = PAR.thetai;
setup.bound.upper.phase(cont).initial.state = [inf,inf,inf];
% setup.bound.upper.phase(cont).final.position = PAR.thetac2;
setup.bound.upper.phase(cont).final.velocity = 50;
setup.bound.upper.phase(cont).final.state = [inf,inf,inf];
...
setup.bound.upper.phase(cont).state = [inf,inf,inf];
setup.bound.upper.phase(cont).control = [MShoulder,MElbow,pi,pi,MShoulder,MElbow,inf,inf,inf,inf];
setup.bound.upper.phase(cont).path = [pi,0,0,0,0,0,Tol];
cont = cont + 1;
...
setup.bound.lower.phase(cont).state = [0,-inf,-inf];
setup.bound.lower.phase(cont).control = [0,0,0,0,-inf,0,0];
setup.bound.lower.phase(cont).path = [0,0,0,-Tol];
setup.phase(cont).mesh_points = 14;
...
setup.bound.upper.phase(cont).state = [inf,inf,inf];
setup.bound.upper.phase(cont).control = [MShoulder,MElbow,MShoulder,MElbow,inf,inf,inf];
setup.bound.upper.phase(cont).path = [pi,0,0,Tol];

setup.bound.lower.parameter = [0,0];
setup.bound.upper.parameter = [0,0];
setup.bound.lower.pconstraints = [zeros(1,19),0.01,0.01,0,Contact];
setup.bound.upper.pconstraints = [zeros(1,19), inf, inf,0,(PAR.thetac2-PAR.thetac1)*0.7];
...
if isnumeric(optr)
	...
	setup.initial_guess.phase(cont).state = [opt.xd1/PAR.Rr,-opt.tau_s1];
	...
	setup.initial_guess.phase(cont).state = [opt.xd2/PAR.Rr,-opt.tau_s2];
	...
	if isfield(optr.solution.phase(1),'state')
		if size(optr.solution.phase(1).state, 2) < 3
            setup.initial_guess.phase(1).state = [optr.solution.phase(1).state(:,1:2),diff_r(optr.solution.phase(1).time,optr.solution.phase(1).velocity)];
        else
            setup.initial_guess.phase(1).state = optr.solution.phase(1).state(:,1:3);
        end
	else
		setup.initial_guess.phase(1).state = [optr.solution.phase(1).velocity*PAR.Rr,zeros(numel(optr.solution.phase(1).velocity),3)];
    end
    if size(optr.solution.phase(1).control, 2) == 10
        setup.initial_guess.phase(1).control = optr.solution.phase(1).control;
    else
	setup.initial_guess.phase(1).control = [optr.solution.phase(1).control(:,1:2), optr.solution.phase(1).control(:,3:4), ...
                                            ...
                                            PAR.Kt*optr.solution.phase(1).state(:,3),optr.solution.phase(1).state(:,3),optr.solution.phase(1).state(:,3)];
    end
    ...
    if size(optr.solution.phase(2).control, 2) == 7
        setup.initial_guess.phase(2).control = optr.solution.phase(2).control;
    else
	setup.initial_guess.phase(2).control = [optr.solution.phase(2).control(:,1:2),optr.solution.phase(2).control(:,3:4), ...
                                            PAR.Kt*optr.solution.phase(2).state(:,3),optr.solution.phase(2).state(:,3),optr.solution.phase(2).state(:,3)];
    end
	if isfield(optr.solution.phase(2),'state')
        if size(optr.solution.phase(2).state, 2) < 3
            setup.initial_guess.phase(2).state = [optr.solution.phase(2).state(:,1:2),diff_r(optr.solution.phase(2).time,optr.solution.phase(2).velocity(:,3))];
        else
            setup.initial_guess.phase(2).state = optr.solution.phase(2).state(:,1:3);
        end
	else
		setup.initial_guess.phase(2).state = [optr.solution.phase(2).velocity*PAR.Rr,zeros(numel(optr.solution.phase(2).velocity),3)];
	end
	setup.initial_guess.parameter =  [1,1];
	output = ddiopt_MB(setup);
end
...
u3 = input.phase(cont).control(:,8);
uv3p = input.phase(cont).control(:,9);
uv3n = input.phase(cont).control(:,10);
uv3 = uv3p-uv3n;

Vimp3 = input.phase(cont).state(:,1);
ierr3 = input.phase(cont).state(:,2);
ddth3 = input.phase(cont).state(:,3);
err3 = Vimp3/input.auxdata.PAR.Rr-dtheta3;
...
% du3 = uv3-u3;
% U3 = Kte*((Jtotal*Ra+La*Btotal)*err3+(Btotal*Ra+Ke-Kuv*du3)*ierr3+La*Jtotal*(1/Td)*(err3-derr3))/Kmotor;
U3 = Kte*(Jtotal*err3+Btotal*ierr3);
...
output.phase(cont).path = [q3(:,1)-q3(:,2), ...
    ...
    input.auxdata.PAR.A*cos(alpha3)+input.auxdata.PAR.B*cos(beta3)-input.auxdata.PAR.R*cos(theta3)-input.auxdata.PAR.h, Tp3 - tp3, du3, U3 - uv3, uv3p*uv3n ];
output.phase(cont).derivatives = [(1/Mi)*(tp3/input.auxdata.PAR.R-Ci*Vimp3-Fri), err3, (1/Td)*(dtheta3-ddth3)];
...
% obj = obj + Kduv*integrate(input.phase(cont).integrand,du3.^2);
...
u4 = input.phase(cont).control(:,5);
uv4p = input.phase(cont).control(:,6);
uv4n = input.phase(cont).control(:,7);
uv4 = uv4p - uv4n;

Vimp4 = input.phase(cont).state(:,1);
ierr4 = input.phase(cont).state(:,2);
ddth4 = input.phase(cont).state(:,3);
err4 = Vimp4/input.auxdata.PAR.Rr-qd4(3);
% duv4 = uv4 - u4;
% U4 = Kte*((Jtotal*Ra+La*Btotal)*err4+(Btotal*Ra+Ke-Kuv*duv4)*ierr4+La*Jtotal*(1/Td)*(err4-derr4))/Kmotor;
U4 = Kte*(Jtotal*err4+Btotal*ierr4);
...
output.phase(cont).path = [q4(:,1)-q4(:,2), du4, U4 - uv4, uv4p*uv4n];
output.phase(cont).derivatives = [(1/Mi)*(-Ci*Vimp4-Fri), err4, (1/Td)*(qd4(3)-ddth4)];
obj = obj + integrate(input.phase(cont).integrand,input.phase(cont).control(:,1).^indice+input.phase(cont).control(:,2).^indice);
obj = obj + integrate(input.phase(cont).integrand,input.phase(cont).control(:,3).^indice+input.phase(cont).control(:,4).^indice);
% obj = obj + Kduv*integrate(input.phase(cont).integrand,duv4.^2);
% obj = obj + integrate(input.phase(cont).integrand,input.phase(cont).control(:,1).^indice+input.phase(cont).control(:,2).^indice)./(input.phase(cont).final.time-input.phase(cont).initial.time);
...
p_constraints = [p_constraints,input.phase(cont).initial.position(3)-input.phase(cont-1).final.position, ...
    ...
    input.phase(cont).final.state(1)-input.phase(cont-1).initial.state(1)+sl2, ...
    input.phase(cont).final.state(2)-input.phase(cont-1).initial.state(2)+sl1, ...
    input.phase(cont).final.state(3)-input.phase(cont-1).initial.state(3), ...
    (1/Td)*(input.phase(cont).final.velocity(3)-input.phase(cont).final.state(3))-(1/Td)*(input.phase(cont-1).initial.velocity-input.phase(cont-1).initial.state(3)), ...
