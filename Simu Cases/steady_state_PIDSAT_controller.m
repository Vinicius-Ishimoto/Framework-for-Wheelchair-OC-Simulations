function output = steady_state_PIDSAT_controller(PAR,speed,varargin)
...
setup.bound.lower.phase(cont).initial.state = [0,-inf,-MCurr,-inf];
...
setup.bound.lower.phase(cont).final.state = [0,-inf,-MCurr,-inf];
...
setup.bound.upper.phase(cont).state = [inf,inf,MCurr,inf];
setup.bound.upper.phase(cont).control = [MShoulder,MElbow,pi,pi,MShoulder,MElbow,inf,inf,inf,inf];
setup.bound.upper.phase(cont).path = [pi,0,0,0,0,0,0];
cont = cont + 1;
...
setup.bound.lower.pconstraints = [zeros(1,20),0.01,0.01,0,Contact];
setup.bound.upper.pconstraints = [zeros(1,20), inf, inf,0,inf];
...
if isnumeric(optr)
	...
	if isfield(setup.initial_guess.phase(1),'state')
		setup.initial_guess.phase(1).state = optr.solution.phase(1).state;
	else
	...
	setup.initial_guess.phase(1).control = [optr.solution.phase(1).control(:,1:2), optr.solution.phase(1).control(:,3:4), ...
                                            ...
                                            optr.solution.phase(1).state(:,3),optr.solution.phase(1).state(:,3),optr.solution.phase(1).state(:,3)];
	...
	setup.initial_guess.phase(2).control = [optr.solution.phase(2).control(:,1:2),optr.solution.phase(2).control(:,3:4), ...
                                            optr.solution.phase(2).state(:,3),optr.solution.phase(2).state(:,3),optr.solution.phase(2).state(:,3)];
	if isfield(setup.initial_guess.phase(2),'state')
		setup.initial_guess.phase(2).state = optr.solution.phase(2).state;
	else
...
i_m3 = input.phase(cont).state(:,3);
derr3 = input.phase(cont).state(:,4);
...
U3 = Kte*((Jtotal*Ra+La*Btotal)*err3+(Btotal*Ra+Ke)*ierr3+La*Jtotal*(1/Td)*(err3-derr3))/Kmotor;
output.phase(cont).RightHandSide = PHI'*(H3*[tau_s3;tau_e3]+ke3-k3)-K+Kmotor*i_m3;
...
Tp3 = M3(3,:)*qdd3-(ke3(3)-k3(3)+Kmotor*i_m3);
...
output.phase(cont).path = [q3(:,1)-q3(:,2), ...
    ...
    input.auxdata.PAR.A*cos(alpha3)+input.auxdata.PAR.B*cos(beta3)-input.auxdata.PAR.R*cos(theta3)-input.auxdata.PAR.h, Tp3 - tp3, U3 - u3, u3 - (uv3p-uv3n), uv3p*uv3n ];
output.phase(cont).derivatives = [(1/Mi)*(tp3/input.auxdata.PAR.R-Ci*Vimp3-Fri), err3, (1/La)*(uv3-Ra*i_m3-Ke*dtheta3), (1/Td)*(err3-derr3)];
...
i_m4 = input.phase(cont).state(:,3);
derr4 = input.phase(cont).state(:,4);
...
U4 = Kte*((Jtotal*Ra+La*Btotal)*err4+(Btotal*Ra+Ke)*ierr4+La*Jtotal*(1/Td)*(err4-derr4))/Kmotor;
...
output.phase(cont).RightHandSide = H4*[tau_s4;tau_e4] + ke4 -k4 + [0;0;Kmotor*i_m4];
output.phase(cont).MassMatrix = M4;
output.phase(cont).path = [q4(:,1)-q4(:,2),U4 - u4, u4 - uv4, uv4p*uv4n];
output.phase(cont).derivatives = [(1/Mi)*(-Ci*Vimp4-Fri), err4, (1/La)*(uv4-Ra*i_m4-Ke*qd4(3)), (1/Td)*(err4-derr4)];
obj = obj + integrate(input.phase(cont).integrand,input.phase(cont).control(:,1).^indice+input.phase(cont).control(:,2).^indice);
...
p_constraints = [p_constraints,input.phase(cont).initial.position(3)-input.phase(cont-1).final.position, ...
    ...
    input.phase(cont).final.state([1,3])-input.phase(cont-1).initial.state([1,3]), ...
    input.phase(cont).final.state([2,4])-input.phase(cont-1).initial.state([2,4])+[sl1,sl2], ...
