function output = MP_muscledyn_PI_controller(PAR,ncycle,velocity,distance,varargin)
...
for i = 0:ncycle
    ...
    setup.bound.lower.phase(cnt).initial.state = [  0,-inf, 0, 0, 0, 0];
	setup.bound.upper.phase(cnt).initial.state = [inf, inf, 1, 1, 1, 1];
    ...
    setup.bound.lower.phase(cnt).final.state = [  0,-inf, 0, 0, 0, 0];
	setup.bound.upper.phase(cnt).final.state = [inf, inf, 1, 1, 1, 1];
    ...
    setup.bound.lower.phase(cnt).state = [-50,-inf, 0, 0, 0, 0];
	setup.bound.upper.phase(cnt).state = [inf, inf, 1, 1, 1, 1];
...
for i = 1:ncycle
    ...
    setup.bound.lower.phase(cnt).state = [  0,-inf, 0, 0, 0, 0];
	setup.bound.upper.phase(cnt).state = [inf, inf, 1, 1, 1, 1];
...
setup.bound.lower.phase(1).initial.state = [ 0, 0, 0, 0, 0, 0];
setup.bound.upper.phase(1).initial.state = [ 0, 0, 0, 0, 0, 0];
...
lb = [];
for i = 1:ncycle
	ub = [ub,zeros(1,17+8),    inf,    inf,    inf,+hFS,+hFT];
	lb = [lb,zeros(1,17+8),mintime,mintime,Contact,-hFS,-hFT];
end
...
for i = 0:ncycle
	...
	if size(opt.solution.phase(cnt).control,2) == 11
		...
		setup.initial_guess.phase(cnt).control(:,1) = opt.solution.phase(cnt).control(:,1)+rand(numl,1);
		setup.initial_guess.phase(cnt).control(:,2) = opt.solution.phase(cnt).control(:,2)+rand(numl,1);
		setup.initial_guess.phase(cnt).control(:,3:4) = opt.solution.phase(cnt).control(:,3:4);
		setup.initial_guess.phase(cnt).control(:,5) = opt.solution.phase(cnt).control(:,5)+rand(numl,1);
		setup.initial_guess.phase(cnt).control(:,6) = opt.solution.phase(cnt).control(:,6)+rand(numl,1);
    ...
    switch opt.Options.Type
        case 'MSC-PI_Controller'
            setup.initial_guess.phase(cnt).state(:,1:2) = [opt.solution.phase(cnt).state];
            setup.initial_guess.phase(cnt).state(:,3) = opt.solution.phase(cnt).control(:,1);
            setup.initial_guess.phase(cnt).state(:,4) = opt.solution.phase(cnt).control(:,2);
            setup.initial_guess.phase(cnt).state(:,5) = opt.solution.phase(cnt).control(:,5);
            setup.initial_guess.phase(cnt).state(:,6) = opt.solution.phase(cnt).control(:,6);
        case 'MSC-Reference'
        ...
        case 'MSD-Reference'
            setup.initial_guess.phase(cnt).state(:,1:2) = [opt.solution.phase(cnt).velocity(:,end)*PAR.Rr+rand(numl,1), opt.solution.phase(cnt).control(:,1)];
            setup.initial_guess.phase(cnt).state(:,3:6) = opt.solution.phase(cnt).state(:,1:4);
        otherwise
...
for i = 1:ncycle
	...
	if size(opt.solution.phase(cnt).control,2) == 8
		...
		setup.initial_guess.phase(cnt).control(:,1) = opt.solution.phase(cnt).control(:,1)+rand(numl,1);
		setup.initial_guess.phase(cnt).control(:,2) = opt.solution.phase(cnt).control(:,2)+rand(numl,1);
		setup.initial_guess.phase(cnt).control(:,3) = opt.solution.phase(cnt).control(:,3)+rand(numl,1);
		setup.initial_guess.phase(cnt).control(:,4) = opt.solution.phase(cnt).control(:,4)+rand(numl,1);
