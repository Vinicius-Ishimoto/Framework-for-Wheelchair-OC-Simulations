...
for q = 1:ncycle
	...
	if hasFixedStart
		ub = [ub,zeros(1,13),  inf,  inf, 0, zeros(1,8),    inf];
		lb = [lb,zeros(1,13),0.1,0.1, 0, zeros(1,8),Contact];
	else
		ub = [ub,zeros(1,13),  inf,  inf, inf, zeros(1,8),    inf];
		lb = [lb,zeros(1,13),0.1,0.1,-inf, zeros(1,8),Contact];
	end
	cont = cont + 1;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
setup.bound.lower.phase(cont).position = PAR.thetac1;
setup.bound.lower.phase(cont).velocity = 0;
setup.bound.lower.phase(cont).state = [-inf,-inf,-MCurr,-inf];
setup.bound.lower.phase(cont).control = [0,0,0,-pi/2,0,0,-inf];
setup.bound.lower.phase(cont).path = [0,0,0,0];
...
obj = obj + fator*integrate(input.phase(cont).integrand,(Kmotor*i_motor1).^2);
cont = cont + 1;
...
obj = obj + fator*integrate(input.phase(cont).integrand,(Kmotor*i_motor2).^2);
...
% obj = obj + integrate(input.phase(cont).integrand,input.phase(cont).control(:,1).^indice+input.phase(cont).control(:,2).^indice)./(input.phase(cont).final.position-input.phase(cont).initial.position);
% obj = obj + integrate(input.phase(cont).integrand,input.phase(cont).control(:,5).^indice+input.phase(cont).control(:,6).^indice)./(input.phase(cont).final.position-input.phase(cont).initial.position);
obj = obj + fator*integrate(input.phase(cont).integrand,(Kmotor*i_motor3).^2);
...
output.objective = obj./(input.phase(cont).final.position-input.phase(1).initial.position);
