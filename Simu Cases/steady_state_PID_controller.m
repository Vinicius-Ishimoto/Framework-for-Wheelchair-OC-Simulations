function output = steady_state_PID_controller(PAR,speed,varargin)
...
output.phase(cont).derivatives = [(1/Mi)*(tp3/input.auxdata.PAR.R-Ci*Vimp3-Fri), err3, (1/La)*(u3-Ra*i_m3-Ke*dtheta3), (1/Td)*(err3-derr3)];
...
output.phase(cont).derivatives = [(1/Mi)*(-Ci*Vimp4-Fri), err4, (1/La)*(u4-Ra*i_m4-Ke*qd4(3)), (1/Td)*(err4-derr4)];
obj = obj + integrate(input.phase(cont).integrand,input.phase(cont).control(:,1).^indice+input.phase(cont).control(:,2).^indice);
