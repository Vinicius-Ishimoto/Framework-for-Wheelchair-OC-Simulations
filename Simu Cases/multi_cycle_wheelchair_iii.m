function output =  multi_cycle_wheelchair_iii(PAR,ncycle,velocity,distance,varargin)
...
for i = 0:ncycle
	...
	setup.bound.lower.phase(cnt).control = [        0,     0,  0*pi/180,-pi/2,        0,     0,-inf];
	setup.bound.upper.phase(cnt).control = [MShoulder,MElbow,180*pi/180,   pi,MShoulder,MElbow,+inf];
	setup.bound.lower.phase(cnt).path = [  0*pi/180,0,0,0];
	setup.bound.upper.phase(cnt).path = [120*pi/180,0,0,0];
...
for i = 1:ncycle
	cnt = 2*i;
	setup.bound.lower.phase(cnt).initial.position = [ 0*pi/180,-pi/2,PAR.thetac1];
	setup.bound.upper.phase(cnt).initial.position = [180*pi/180,pi,PAR.thetac2];
	...
	setup.bound.lower.phase(cnt).final.position = [ 0*pi/180,-pi/2,PAR.thetac1];
	setup.bound.upper.phase(cnt).final.position = [180*pi/180,pi,inf];
	...
	setup.bound.upper.phase(cnt).final.time = inf;
	setup.bound.lower.phase(cnt).position = [ 0*pi/180,-pi/2,PAR.thetac1];
	setup.bound.upper.phase(cnt).position = [180*pi/180,pi,inf];
	...
	setup.bound.lower.phase(cnt).path =   0*pi/180;
	setup.bound.upper.phase(cnt).path = 120*pi/180;
...
lb = [];
for i = 1:ncycle
	ub = [ub,zeros(1,13),    inf,    inf,    inf,+hFS];
	lb = [lb,zeros(1,13),mintime,mintime,Contact,-hFS];
end
setup.bound.lower.pconstraints = [lb,  0.1,velocity,  1,-hFS];
setup.bound.upper.pconstraints = [ub,  inf,velocity,inf,+hFS];
...
for i = 0:ncycle
	...
	if size(opt.solution.phase(cnt).control,2) == 7
		setup.initial_guess.phase(cnt).control = [opt.solution.phase(cnt).control];
    elseif size(opt.solution.phase(cnt).control,2) == 6
		setup.initial_guess.phase(cnt).control = [opt.solution.phase(cnt).control,opt.solution.phase(cnt).taup];
    elseif size(opt.solution.phase(cnt).control,2) == 11
		setup.initial_guess.phase(cnt).control(:,1:2) = opt.solution.phase(cnt).control(:,8:9);
        setup.initial_guess.phase(cnt).control(:,3:4) = opt.solution.phase(cnt).control(:,3:4);
        setup.initial_guess.phase(cnt).control(:,5:6) = opt.solution.phase(cnt).control(:,10:11);
        setup.initial_guess.phase(cnt).control(:,7) = opt.solution.phase(cnt).control(:,7);
	else
		setup.initial_guess.phase(cnt).control = [opt.solution.phase(cnt).control,opt.solution.phase(cnt).taup,-opt.solution.phase(cnt).control(:,1:2)];    
    end
end
...
for i = 1:ncycle
	...
	if size(opt.solution.phase(cnt).control,2) == 4
		setup.initial_guess.phase(cnt).control = [opt.solution.phase(cnt).control];
    elseif size(opt.solution.phase(cnt).control,2) == 8
        setup.initial_guess.phase(cnt).control(:,1:4) = opt.solution.phase(cnt).control(:,5:8);
	else
