function out = multi_cycle_PI_integrate(PAR,opt,OnlyPos)
...
if rem(q,2)
	...
	if q==1
		...
		[T1,Y1] = ode113(@(tv,vt) dynamics_int_erri(vt,interp1(rtheta,Vtau,vt(1),'spline','extrap'),PAR,const,1),[time(1),time(end)],[opt.solution.phase(q).position(1);opt.solution.phase(q).velocity(1);opt.solution.phase(q).velocity(1)*PAR.Rr;0;0],odeset('Events',@(tv,vt) EventsFcn(tv,vt,PAR,opt.solution.phase(q).position(end))));
	else
	...
	for i=1:size(Y1,1)
        ...
        tarray = dynamics_int_erri(Y1(i,:),interp1(rtheta,Vtau,Y1(i,1),'spline','extrap'),PAR,2e2,0);
		tp = [tp;tarray(1)];
        U1 = [U1;tarray(2)];
        U1p = [U1p;tarray(3)];
        U1n = [U1n;tarray(4)];
	...
	out.solution.phase(q).control = [tau(:,1:2),alpha,beta,tau(:,3:4),tp,U1,U1p,U1n];
	out.solution.phase(q).position = Y1(:,1);
	out.solution.phase(q).velocity = Y1(:,2);
	out.solution.phase(q).state = Y1(:,3:5);
...
err = V/PAR.Rr-dtheta;
[M,ke,~,H,k] = four_bar_system_no_motor(q, qd,PAR);
MM = PHI'*M*PHI;
K = PHI'*M*dPHI*dtheta;
...
KE = PHI'*(H*[u(1)-u(3);u(2)-u(4)]+ke-k)+U;
ddtheta = MM\(KE-K);
...
setup.bound.lower.phase.final.state = [0,-inf,-inf];
...
setup.bound.lower.phase.state = [0,-inf,-inf];
setup.bound.lower.phase.control = [0,0,0,0,-inf,0,0];
setup.bound.lower.phase.path = [0,0,0,-1];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
setup.bound.upper.phase.final.state = [inf,inf,inf];
...
setup.bound.upper.phase.state = [inf,inf,inf];
setup.bound.upper.phase.control = inf(1,7);
setup.bound.upper.phase.path = [pi,0,0,1];
...
setup.initial_guess.phase.state = [rg.solution.phase(q).velocity(:,3)*PAR.Rr,rg.solution.phase(q).control(:,1:2)];
...
output.phase.path = [q2(:,1)-q2(:,2), du2, U2 - uv2, uv2p*uv2n];

ddx2 = (1/Mi)*(-Ci*v_imp2-Fi);
output.phase.derivatives = [ddx2,err2, (1/input.auxdata.PAR.Td)*(qd2(3)-ddth2)];
