...
Person.Reference = wheelchair_parameters('height',Person.Height, 'mass',Person.Mass, 'angle', 5);
Results.Reference = steady_state_wheelchair(Person.Reference, Speed, Options{:});
...
% 	Guess = multi_cycle_PID_integrate(Person.Controller{q}, Results.Reference);
	...
	Options = {'Max_shoulder',Max_Shoulder,'Max_elbow',Max_Elbow,'Guess',Guesses1.Results.Controller{q}, 'indice', Indice};
    ...
    Person.Controller{q} = wheelchair_parameters('height',Person.Height, 'mass',Person.Mass, ...
        'Mi', ReferenceModel.Mass*Simulations(q,1), 'Ci', ReferenceModel.Friction*Simulations(q,2), ...
        'Mr', ReferenceModel.Mass, 'Cr', ReferenceModel.Friction, 'angle', 5);
    
    Options = {'Max_shoulder',Max_Shoulder,'Max_elbow',Max_Elbow,'Guess',Guesses2.Results.Controller{q}, 'indice', Indice};
	Results.Controller{q} = steady_state_PI_controller(Person.Controller{q}, Speed, Options{:});
...
save([cd,'\Results\','Indice',int2str(Indice),'_SteadyState','.mat'],'Results','Person','ReferenceModel','Options','Simulations','FDATA')
